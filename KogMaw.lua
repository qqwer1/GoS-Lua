...
-- SpellDMG
if mainMenu.Misc.drawDMGspell:Value() and ValidTarget(enemy,(300*GetCastLevel(myHero,_R)+900)) then
	if CanUseSpell(myHero,_Q) == READY then
		qDMG = CalcDamage(myHero,enemy,0,50*GetCastLevel(myHero,_Q)+30+(0.5*GetBonusAP(myHero)))
	else qDMG = 0 end
	if CanUseSpell(myHero,_E) == READY then
		eDMG = CalcDamage(myHero,enemy,0,50*GetCastLevel(myHero,_E)+10+(0.7*GetBonusAP(myHero)))
	else eDMG = 0 end
	if CanUseSpell(myHero,_R) == READY then
		rDMG = CalcDamage(myHero,enemy,0,80*GetCastLevel(myHero,_R)+80+(0.3*GetBonusAP(myHero))+(0.5*(GetBaseDamage(myHero)+GetBonusDmg(myHero))))
	else rDMG = 0 end
...
-- AA DMG
if mainMenu.Misc.drawDMG:Value() and ValidTarget(enemy,(300*GetCastLevel(myHero,_R)+900)) then
local bork = GetItemSlot(myHero,3153)
	AADMG = CalcDamage(myHero,enemy,GetBaseDamage(myHero)+GetBonusDmg(myHero),0)
	if GotBuff(myHero,"KogMawBioArcaneBarrage") == 1 then
		extraDMG = CalcDamage(myHero,enemy,0,GetMaxHP(enemy)*(0.01*GetCastLevel(myHero,_W)+0.01+(0.01*math.floor(0.01*GetBonusAP(myHero)))))
	else extraDMG = 0 end
	if bork >= 1 then
		borkDMG = CalcDamage(myHero,enemy,GetCurrentHP(enemy)*0.06,0)
	else borkDMG = 0 end
...
if mainMenu.Misc.drawW:Value() and CanUseSpell(myHero,_W) == READY and ValidTarget(GetCurrentTarget(),2000) then
	DrawCircle(GetOrigin(myHero),GetRange(myHero)+(20*GetCastLevel(myHero,_W)+110+GetHitBox(myHero)),2,100,ARGB(210,33,139,6))
end
...
-- Items ---------------------------
	...
	if ghost >= 1 and ValidTarget(target,GetRange(myHero)+GetHitBox(myHero)+GetHitBox(target)) and mainMenu.Items.useGhost:Value() then
		if CanUseSpell(myHero,GetItemSlot(myHero,3142)) == READY then
			CastSpell(GetItemSlot(myHero,3142))
		end
	end
	
	if redpot >= 1 and ValidTarget(target,GetRange(myHero)+GetHitBox(myHero)+GetHitBox(target)) and mainMenu.Items.useRedPot:Value() then
...
if mainMenu.Combo.useQ:Value() then
	if CanUseSpell(myHero,_Q) == READY and ValidTarget(target, GetRange(myHero)) then
...
if mainMenu.Combo.useW:Value() then
	if CanUseSpell(myHero,_W) == READY and ValidTarget(target, GetRange(myHero)+(20*GetCastLevel(myHero,_W)+110)+GetHitBox(myHero)+GetHitBox(target)) then
		CastSpell(_W)
	end
end
-- E
if mainMenu.Combo.useE:Value() then
	if CanUseSpell(myHero,_E) == READY and ValidTarget(target, 1280) then 
...
if mainMenu.Combo.useR:Value() then
	if CanUseSpell(myHero,_R) == READY and ValidTarget(target,(300*GetCastLevel(myHero,_R)+900)) and GotBuff(myHero,"kogmawlivingartillerycost") < mainMenu.Combo.RStacks:Value() then
...
if mainMenu.Harass.useR:Value() and 100*GetCurrentMana(myHero)/GetMaxMana(myHero) >= mainMenu.Harass.Mana:Value() then
	if CanUseSpell(myHero,_R) == READY and ValidTarget(target,(300*GetCastLevel(myHero,_R)+900)) and GotBuff(myHero,"kogmawlivingartillerycost") < mainMenu.Harass.RStacks:Value() then
		local RPred = GetPredictionForPlayer(myHeroPos,target,GetMoveSpeed(target),math.huge,1200,(300*GetCastLevel(myHero,_R)+900),100,false,false)
		if RPred.HitChance == 1 then
			CastSkillShot(_R,RPred.PredPos.x,RPred.PredPos.y,RPred.PredPos.z)
		end
...
for i,enemy in pairs(GetEnemyHeroes()) do
	if mainMenu.Killsteal.ksQ:Value() and CanUseSpell(myHero,_Q) == READY and ValidTarget(enemy,GetCastRange(myHero,_Q)) and GetCurrentHP(enemy) + GetDmgShield(enemy) + GetMagicShield(enemy) < CalcDamage(myHero,enemy,0,50*GetCastLevel(myHero,_Q)+30+(0.5*GetBonusAP(myHero))) then
	...
	if mainMenu.Killsteal.ksE:Value() and CanUseSpell(myHero,_E) == READY and ValidTarget(enemy,GetCastRange(myHero,_E)) and GetCurrentHP(enemy) + GetDmgShield(enemy) + GetMagicShield(enemy) < CalcDamage(myHero,enemy,0,50*GetCastLevel(myHero,_E)+10+(0.7*GetBonusAP(myHero))) then
	...
	if mainMenu.Killsteal.ksR:Value() and CanUseSpell(myHero,_R) == READY and ValidTarget(enemy,(300*GetCastLevel(myHero,_R)+900)) and GetCurrentHP(enemy) + GetDmgShield(enemy) + GetMagicShield(enemy) < CalcDamage(myHero,enemy,0,80*GetCastLevel(myHero,_R)+80+(0.3*GetBonusAP(myHero))+(0.5*(GetBaseDamage(myHero)+GetBonusDmg(myHero)))) then
