...
if mainMenu.Combo.AutoE:Value() then
	if CanUseSpell(myHero,_E) == READY and GoS:ValidTarget(target, 290) and mainMenu.Combo.AutoE:Value() then
		CastSpell(_E)
	end
-- W _E	
	if CanUseSpell(myHero,_E) == READY and mainMenu.Combo.AutoE:Value() and WPos ~= nil then
		local wEPred = GetPredictionForPlayer(WPos,target,GetMoveSpeed(target),math.huge,250,290,0,false,false)
...
-- R _E	
	if CanUseSpell(myHero,_E) == READY and mainMenu.Combo.AutoE:Value() and RPos ~= nil then
		local rEPred = GetPredictionForPlayer(RPos,target,GetMoveSpeed(target),math.huge,250,290,0,false,false)
...
--- Items
	...
	if ghost >= 1 and GoS:ValidTarget(target,550) and CanUseSpell(myHero,GetItemSlot(myHero,3142)) == READY and mainMenu.Items.useGhost:Value() then
...
if mainMenu.Combo.useW:Value() then
	if CanUseSpell(myHero,_W) == READY and GoS:ValidTarget(target, GetCastRange(myHero,_W)+GetRange(myHero)) and not GoS:IsInDistance(target, GetRange(myHero)) and mainMenu.Combo.gabW:Value() then
...
-- Q
	if CanUseSpell(myHero,_Q) == READY and GoS:ValidTarget(target, GetCastRange(myHero,_Q)) and mainMenu.Combo.useQ:Value() then
...
-- QW
	if CanUseSpell(myHero,_Q) == READY and WPos ~= nil and GoS:ValidTarget(target, 4000) and mainMenu.Combo.useQ:Value() then
...
-- QR
	if CanUseSpell(myHero,_Q) == READY and RPos ~= nil and GoS:ValidTarget(target, 4000) and mainMenu.Combo.useQ:Value() then
...
-- E
	if CanUseSpell(myHero,_E) == READY and GoS:ValidTarget(target, GetCastRange(myHero,_E)) and GoS:IsInDistance(target, GetCastRange(myHero,_E)) and mainMenu.Combo.useE:Value() then
		CastSpell(_E)
	end
-- EW
	if CanUseSpell(myHero,_E) == READY and WPos ~= nil and GoS:ValidTarget(target, 4000) and mainMenu.Combo.useE:Value() then
		local wEPred = GetPredictionForPlayer(WPos,target,GetMoveSpeed(target),math.huge,250,290,0,false,false)
			if wEPred.HitChance == 1 then
				CastSpell(_E)
			end
...
-- ER
	if CanUseSpell(myHero,_E) == READY and RPos ~= nil and GoS:ValidTarget(target, 4000) and mainMenu.Combo.useE:Value() then
		local rEPred = GetPredictionForPlayer(RPos,target,GetMoveSpeed(target),math.huge,250,290,0,false,false)
			if rEPred.HitChance == 1 then
				CastSpell(_E)
			end
