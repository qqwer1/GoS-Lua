...
if mainMenu.Drawings.DrawDMG:Value() and CanUseSpell(myHero,_R) == READY and ValidTarget(target, 1500) then
	local DPS = CalcDamage(myHero,target,(10*GetCastLevel(myHero,_R)+30+(0.25*(GetBaseDamage(myHero) + GetBonusDmg(myHero))) * (7 + (((150*GetCastLevel(myHero,_R)+500) + GetAttackSpeed(myHero)) / (22.4 - 2.49*(GetCastLevel(myHero,_R)))) * ((22.4 - 2.49*(GetCastLevel(myHero,_R))) / 100))),0)
...
----------------------------
if GotBuff(myHero,"lucianpassivebuff") == 1 or Passive == 1 and ValidTarget(target, 550) then
	AttackUnit(target)
...
if CanUseSpell(myHero,_Q) == READY and mainMenu.Combo.useQ:Value() and GotBuff(myHero,"lucianpassivebuff") == 0 then
		if ValidTarget(target, GetRange(myHero)+GetHitBox(target)) then
